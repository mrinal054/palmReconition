...
trn = dlmread('DCT_32x32_train.txt');           % 80x200 % Column-wise data 
s_input = trn(:,1:P)';                           
% 200x80 % Row-wise data
tst = dlmread('DCT_32x32_test.txt');            % 80x200
s_test = tst(:,1:Pt)';
...
for i = 1:TA    
    ...
    [index,Cx]=kmeans(DC,Hc);               % size(Cx) = HcxN; 3x80 
    C(valc:valc+Hc-1,:) = Cx;               % size(C) = H1xN; 120x80  
...
for i = 1:P
    ...
    valc = 1;
    for j = 1:TA
        Hb00(j) = sum(Ha(valc:valc+Hc-1));    % sum A.Fs of each class, 1xTA-->1x40
        Hb(j)=(Hb00(j)*U);
        valc = valc + Hc;
    end
    clear valc
...
for i = 1:Pt
    ...
    valc = 1;
    for j = 1:TA
        Hbt11(j) = sum(Hat(valc:valc+Hc-1));    % sum A.Fs of each class, 1xTA-->1x40
        Hbt (j)= Hbt11(j)*U ;
        valc = valc + Hc;
    end
    clear valc
